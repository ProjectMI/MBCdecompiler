...
class Disassembler:
    ...
    def _format_instruction(self, instruction: InstructionWord) -> str:
        ...
        stack_detail = "stackΔ="
        if metadata.stack_delta is not None:
            stack_detail += f"{metadata.stack_delta:+.1f}"
            extras = []
            if metadata.stack_confidence is not None:
                extras.append(f"{metadata.stack_confidence * 100:.0f}%")
