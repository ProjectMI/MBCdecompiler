...
class IRNormalizer:
    ...
    def _collapse_tail_return(self, items: _ItemList, call_index: int, metrics: NormalizerMetrics) -> None:
        ...
        while index < len(items):
            ...
            if isinstance(item, RawInstruction) and item.profile.kind in {
                ...
                items.pop(index)
                continue
            if self._is_stack_cleanup_spacer(item):
                index += 1
                continue
            break
        ...
        while scan >= 0:
            ...
            if isinstance(candidate, RawInstruction):
                ...
                if kind is InstructionKind.STACK_TEARDOWN:
                    ...
                    scan -= 1
                    continue
                if self._is_stack_cleanup_spacer(candidate):
                    scan -= 1
                    continue
    ...
    def _pass_call_cleanup(self, items: _ItemList) -> None:
        ...
        while index < len(items):
            ...
            while prev_index >= 0 and self._is_stack_cleanup_spacer(items[prev_index]):
                prev_index -= 1

            if prev_index >= 0 and isinstance(items[prev_index], IRCall):
                items.insert(insertion_index, IRCallCleanup(steps=tuple(steps)))
                index = insertion_index + 1
                continue

            next_index = insertion_index
            while next_index < len(items) and self._is_stack_cleanup_spacer(items[next_index]):
                next_index += 1
        ...
        while index < len(items) - 1:
            ...
            if isinstance(call, IRCall):
                ...
                if offset < len(items) and isinstance(items[offset], IRReturn):
                    ...
                    prefix: List[Union[RawInstruction, IRNode]] = []
                    suffix: List[Union[RawInstruction, IRNode]] = []
                    for pos in range(index + 1, end):
                        candidate = items[pos]
                        if cleanup_index is not None and pos >= cleanup_index:
                    ...
                    items.replace_slice(index, end, prefix + [node] + suffix)
                    continue
            index += 1
    ...
    def _is_stack_cleanup_spacer(self, item: Union[RawInstruction, IRNode]) -> bool:
        if isinstance(item, (IRLiteral, IRLiteralChunk, IRLiteralBlock)):
            return True
        if isinstance(item, (IRBuildArray, IRBuildMap, IRBuildTuple)):
            return True
        if isinstance(item, RawInstruction):
            if self._is_annotation_only(item):
                return True
