...
class IRNormalizer:
    ...
    def _normalise_block(self, block: RawBlock) -> Tuple[IRBlock, NormalizerMetrics]:
        ...
        self._pass_tail_helper_epilogues(items)
    ...
    def _pass_tail_helpers(self, items: _ItemList) -> None:
        ...
        while index < len(items):
            ...
            cleanup_step: Optional[IRStackEffect] = None
            updated: Optional[Union[IRCall, IRCallReturn, IRTailcallReturn, IRReturn]] = None
            if helper_target == 0x0072:
                updated, cleanup_step = self._rewrite_tail_helper_72(item, new_target)
            else:
                updated = self._rewrite_tail_helper_port(item, new_target)
            ...
            if cleanup_step is not None:
                self._attach_tail_cleanup(items, index + 1, cleanup_step)
    ...
    def _rewrite_tail_helper_72(
        ...
        target: int,
    ) -> Tuple[Optional[Union[IRCall, IRCallReturn, IRTailcallReturn, IRReturn]], Optional[IRStackEffect]]:
        shuffle_operand: Optional[int] = None
        if getattr(node, "convention", None) is not None:
            convention = node.convention
            if convention and convention.mnemonic == "stack_shuffle":
                shuffle_operand = convention.operand

        args = getattr(node, "args", tuple())
        applied_args = self._apply_tail_shuffle(args, shuffle_operand)

        cleanup_step: Optional[IRStackEffect] = None
        remaining_cleanup: Tuple[IRStackEffect, ...] = tuple()
        for step in getattr(node, "cleanup", tuple()):
            if step.mnemonic == "stack_teardown" and cleanup_step is None:
                cleanup_step = IRStackEffect(
                    mnemonic=step.mnemonic,
                    operand=step.operand,
                    pops=step.pops,
                    operand_role=step.operand_role,
                    operand_alias=step.operand_alias or "tail_helper_72",
                )
                continue
            remaining_cleanup += (step,)

        if cleanup_step is None:
            cleanup_step = IRStackEffect(
                mnemonic="stack_teardown",
                pops=1,
                operand_alias="tail_helper_72",
            )

        symbol = self.knowledge.lookup_address(target)
        cleanup_mask = getattr(node, "cleanup_mask", None)
        tail = getattr(node, "tail", False) or cleanup_mask == RET_MASK
        ...
        return updated, cleanup_step

    def _rewrite_tail_helper_port(
        self,
        node: Union[IRCall, IRCallReturn, IRTailcallReturn],
        target: int,
    ) -> Optional[Union[IRCall, IRCallReturn, IRTailcallReturn]]:
        symbol = self.knowledge.lookup_address(target)
        if isinstance(node, IRCallReturn):
            return IRCallReturn(
                target=target,
    ...
    def _attach_tail_cleanup(
        self, items: _ItemList, index: int, step: IRStackEffect
    ) -> None:
        if index < len(items) and isinstance(items[index], IRCallCleanup):
            cleanup_node = items[index]
            updated = IRCallCleanup(steps=cleanup_node.steps + (step,))
            self._transfer_ssa(cleanup_node, updated)
            items.replace_slice(index, index + 1, [updated])
            return

        items.insert(index, IRCallCleanup(steps=(step,)))

    @staticmethod
    def _apply_tail_shuffle(args: Tuple[str, ...], operand: Optional[int]) -> Tuple[str, ...]:
        if operand is None or not args:
            return args
    ...
    def _pass_tail_helper_epilogues(self, items: _ItemList) -> None:
        index = 0
        while index < len(items):
            node = items[index]
            if not isinstance(node, IRTailcallReturn):
                index += 1
                continue
            ...
            teardown_steps = [
                ...
                if step.mnemonic == "stack_teardown" and step.operand_alias == "tail_helper_72"
            ]
            if not teardown_steps:
                index += 1
                continue

            remaining = tuple(step for step in node.cleanup if step not in teardown_steps)
            updated = IRTailcallReturn(
                target=node.target,
                args=node.args,
                returns=node.returns,
            ...
            for step in teardown_steps:
                self._attach_tail_cleanup(items, index + 1, step)
