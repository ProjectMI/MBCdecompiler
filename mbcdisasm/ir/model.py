...
class IRTableBuilderBegin(IRNode):
    ...
    annotations: Tuple[str, ...] = field(default_factory=tuple)
    metadata_ops: Tuple[Tuple[str, int], ...] = field(default_factory=tuple)
    ...
    def describe(self) -> str:
        ...
        if self.metadata_ops:
            affix = ", ".join(
                f"{mnemonic}(0x{operand:04X})" for mnemonic, operand in self.metadata_ops
            )
            details.append(f"metadata=[{affix}]")
        if self.parameters:
        ...
        if self.annotations:
            details.append(", ".join(self.annotations))
...
class IRTableBuilderCommit(IRNode):
    ...
    def describe(self) -> str:
        ...
        return "table_commit " + " ".join(details)
