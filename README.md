...
This repository now focuses on a single task: analysing Sphere `.mbc` script containers and lifting their bytecode into progressively richer representations.  The command-line interface reads a container plus its `.adb` index file, runs the structural analysis pipeline, and emits both a normalised IR listing and an AST summary that mirror the internal data structures.
...
```
python mbc_disasm.py <adb> <mbc> [--segment INDEX ...] [--max-instr COUNT] [--ir-out PATH] [--ast-out PATH]
```
...
The CLI prints the final output paths and writes both the IR listing and AST summary to disk.
