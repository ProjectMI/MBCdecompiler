...
def test_normalizer_builds_ir(tmp_path: Path) -> None:
    ...
    assert cleanup_mnemonics == ["stack_teardown", "op_29_10", "op_70_29"]
    assert contract_call.cleanup[1].operand == 0x2910
...
def test_tail_helper_wrappers_collapse(tmp_path: Path) -> None:
    ...
    tail_io_nodes = [
        ...
        and getattr(node.call, "symbol", None) == "io.write"
    ]
    assert tail_io_nodes
    cleanup_nodes = [node for node in flattened if isinstance(node, IRCallCleanup)]
    assert any(
        any(
            effect.mnemonic == "op_3D_30" and effect.operand_alias == "IO_SLOT"
            for effect in cleanup.steps
        )
        for cleanup in cleanup_nodes
    )
    for node in tail_io_nodes:
        assert any(effect.mnemonic == "op_4C_00" for effect in node.cleanup)
...
def test_normalizer_collapses_io_facade_helpers(tmp_path: Path) -> None:
    ...
    assert [step.mnemonic for step in cleanup.steps[:2]] == [
        "stack_teardown",
        "call_helpers",
    ]
...
def test_ascii_bridge_with_side_effect_operand_remains_raw() -> None:
    ...
    assert any(step.mnemonic == "op_AA_00" for step in cleanup.steps)
...
def test_normalizer_handles_extended_io_variants(tmp_path: Path) -> None:
    ...
    tail = tail_calls[0]
    assert any(step.mnemonic == "op_10_84" for step in tail.cleanup)
    assert any(
        step.mnemonic == "op_3D_30" and step.operand_alias == "IO_SLOT"
        for step in tail.cleanup
    )
...
def test_normalizer_groups_call_helper_cleanup(tmp_path: Path) -> None:
    ...
    assert any(step.operand == 0x2910 for step in call_return.cleanup)
...
def test_normalizer_collapses_f0_tailcall(tmp_path: Path) -> None:
    ...
    assert any(
        step.mnemonic == "op_29_10" and step.operand == RET_MASK
        for step in tail_node.cleanup
    )
