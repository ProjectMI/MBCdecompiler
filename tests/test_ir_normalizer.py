...
def test_tail_helper_wrappers_collapse(tmp_path: Path) -> None:
    ...
    helper_3d = next(node for node in tail_nodes if node.target == 0x3D30)
    helper_72 = next(node for node in tail_nodes if node.target == 0x3032)

    assert helper_3d.cleanup == ()
    assert helper_3d.cleanup_mask is None
    assert helper_3d.convention is None

    assert helper_72.cleanup == ()
    assert helper_72.cleanup_mask is None
    assert helper_72.convention is None

    cleanup_after_helper = [
        node
        for idx, node in enumerate(flattened[:-1])
        if node is helper_72 and isinstance(flattened[idx + 1], IRCallCleanup)
    ]
    assert cleanup_after_helper
    teardown_cleanup = flattened[flattened.index(helper_72) + 1]
    assert isinstance(teardown_cleanup, IRCallCleanup)
    assert any(step.mnemonic == "stack_teardown" for step in teardown_cleanup.steps)
