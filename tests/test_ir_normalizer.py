...
def test_normalizer_builds_table_pipeline_nodes() -> None:
    ...
    assert begin.mode_descriptor is None

    assert isinstance(emit, IRTableBuilderEmit)
    assert emit.kind == "adaptive_table.mode_4D.39"
    ...
    assert commit.commit == 0x0000
    assert commit.fallback == 0x1000
    assert commit.annotations == ("guard=table_patch",)


def test_normalizer_table_builder_absorbs_auxiliary_prologue() -> None:
    knowledge = KnowledgeBase({})
    normalizer = IRNormalizer(knowledge)

    prologue_a = make_stack_neutral_instruction(0, "op_39_4D")
    prologue_b = make_stack_neutral_instruction(2, "op_3F_4D")
    header = IRLiteralChunk(data=b"HEAD", source="test", symbol="str_hdr")
    aux = make_stack_neutral_instruction(4, "op_5D_4D")
    descriptor = IRLiteralChunk(data=b"DESC", source="test", symbol="str_desc")
    table = IRTablePatch(
        operations=(("op_82_4D", 0x0000), ("op_88_4D", 0x0000)),
    ...
    follower = IRIf(
        condition="table_patch adaptive_table, mode=0x4D",
        then_target=0x0000,
        else_target=0x1000,
    )

    items = _ItemList([prologue_a, prologue_b, header, aux, descriptor, table, guard, follower])
    normalizer._pass_table_builders(items)
    ...
    assert isinstance(begin, IRTableBuilderBegin)
    assert begin.prologue == (
        ("op_39_4D", 0x0000),
        ("op_3F_4D", 0x0000),
        ("op_5D_4D", 0x0000),
    )
    assert begin.header == "str(str_hdr)"
    assert begin.mode_descriptor == "str(str_desc)"

    assert isinstance(emit, IRTableBuilderEmit)
    assert emit.kind.startswith("adaptive_table.mode_4D")
    assert emit.parameters == tuple()
