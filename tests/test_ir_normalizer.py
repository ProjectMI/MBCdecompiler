...
def test_normalizer_folds_call_cleanup_through_spacers(tmp_path: Path) -> None:
    knowledge = write_manual(tmp_path)

    words = [
        build_word(0, 0x28, 0x00, 0x1234),  # call_dispatch
        build_word(4, 0x00, 0x38, 0x9999),  # literal_marker (annotation only)
        build_word(8, 0x01, 0xF0, 0x0000),  # stack_teardown_4
        build_word(12, 0x00, 0x38, 0x0067),  # literal_marker -> IRLiteral
...
def test_normalizer_merges_return_cleanup_spacers(tmp_path: Path) -> None:
    knowledge = write_manual(tmp_path)

    words = [
        build_word(0, 0x00, 0x38, 0x9999),  # literal_marker (annotation only)
        build_word(4, 0x01, 0xF0, 0x0000),  # stack_teardown_4
        build_word(8, 0x00, 0x38, 0x0067),  # literal_marker -> IRLiteral
    ...
    assert len(ret.cleanup) == 2
    assert all(effect.mnemonic == "stack_teardown" for effect in ret.cleanup)
    assert all(effect.pops == 4 for effect in ret.cleanup)
    assert not any(
        isinstance(node, IRRaw) and node.mnemonic.startswith("stack_teardown")
        for node in block.nodes
    )
...
def test_normalizer_inlines_shuffle_past_annotations(tmp_path: Path) -> None:
    knowledge = write_manual(tmp_path)
