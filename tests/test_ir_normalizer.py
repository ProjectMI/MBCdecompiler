...
def test_tail_helper_wrappers_collapse(tmp_path: Path) -> None:
    ...
    assert any(tail.call.symbol == "io.write" for tail in tail_calls)
    io_tail = next(
        tail for tail in tail_calls if tail.call.symbol == "io.write"
    )
    assert [step.mnemonic for step in io_tail.cleanup] == ["op_4C_00"]
...
def test_normalizer_handles_direct_io_sequences(tmp_path: Path) -> None:
    ...
    assert any(
        any(step.mnemonic == "op_3D_30" for step in cleanup.steps)
...
def test_normalizer_collapses_io_facade_helpers(tmp_path: Path) -> None:
    ...
    cleanup = next(
        node for node in block.nodes if isinstance(node, IRCallCleanup)
    )
    assert [step.mnemonic for step in cleanup.steps] == [
        "stack_teardown",
        "call_helpers",
        "op_13_00",
    ]
    assert [step.operand for step in cleanup.steps] == [0x0029, 0x3E4B, 0x3069]

    io_writes = [node for node in block.nodes if isinstance(node, IRIOWrite)]
    assert len(io_writes) == 1

    write = io_writes[0]
    assert [step.mnemonic for step in write.pre_helpers] == ["call_helpers"]
    assert [step.operand for step in write.pre_helpers] == [0x0100]
    assert [step.mnemonic for step in write.post_helpers] == ["op_F0_4B", "op_4A_10"]
...
def test_ascii_bridge_with_side_effect_operand_remains_raw() -> None:
    ...
    assert any(
        ...
        and [step.mnemonic for step in node.steps] == ["op_AA_00"]
...
def test_normalizer_handles_extended_io_variants(tmp_path: Path) -> None:
    ...
    tail_blocks = [
        block
        for block in blocks
        if any(isinstance(node, IRTailCall) for node in block.nodes)
    ]
    assert tail_blocks
    tail_node = next(
        node
        for node in tail_blocks[0].nodes
        if isinstance(node, IRTailCall)
    )
    assert [step.mnemonic for step in tail_node.cleanup] == [
        "op_10_84",
        "op_3D_30",
    ]
    ...
    cleanup_block = next(
        block
        for block in blocks
        if any(isinstance(node, IRCallCleanup) for node in block.nodes)
    )
    cleanup_node = next(
        node for node in cleanup_block.nodes if isinstance(node, IRCallCleanup)
    )
    assert [step.mnemonic for step in cleanup_node.steps] == ["op_3D_30"]
    ret_node = next(
        node for node in cleanup_block.nodes if isinstance(node, IRReturn)
    )
    assert [step.mnemonic for step in ret_node.cleanup] == ["op_0C_09", "op_10_50"]
...
def test_condition_mask_from_ret_mask_literal(tmp_path: Path) -> None:
    ...
    assert [step.operand for step in node.steps] == [RET_MASK]
...
def test_normalizer_collapses_f0_tailcall(tmp_path: Path) -> None:
    ...
    assert tail_node.cleanup_mask is None
