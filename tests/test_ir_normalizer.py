...
def test_normalizer_call_cleanup_skips_ascii_noise(tmp_path: Path) -> None:
    knowledge = write_manual(tmp_path)

    words = [
        build_word(0, 0x28, 0x00, 0x0042),  # call_dispatch
        build_ascii_word(4, "NOIS"),
    ...
    assert any(
        "ascii(" in getattr(node, "describe", lambda: "")()
        for node in block.nodes
    )

    cleanup = next(node for node in block.nodes if isinstance(node, IRCallCleanup))
    assert cleanup.steps
    assert cleanup.steps[-1].mnemonic == "stack_teardown"
    assert cleanup.steps[-1].pops == 4
    assert not any(
...
def test_normalizer_call_preparation_skips_ascii_noise(tmp_path: Path) -> None:
    knowledge = write_manual(tmp_path)

    words = [
        build_word(0, 0x66, 0x15, 0x4B08),  # stack_shuffle
        build_ascii_word(4, "FILL"),
        build_word(8, 0x28, 0x00, 0x0020),  # call_dispatch
        build_word(12, 0x30, 0x00, 0x0000),  # return_values
    ]
    ...
    prep = next(node for node in block.nodes if isinstance(node, IRCallPreparation))
    assert [step[0] for step in prep.steps] == ["stack_shuffle"]
    assert not any(
        isinstance(node, IRRaw) and node.mnemonic == "stack_shuffle" for node in block.nodes
    )
...
def test_normalizer_return_cleanup_skips_ascii_noise(tmp_path: Path) -> None:
    knowledge = write_manual(tmp_path)
