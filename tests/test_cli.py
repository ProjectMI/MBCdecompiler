...
def test_cli_generates_ast(tmp_path: Path) -> None:
    ...
    assert "ast written" in result.stdout
    ...
    ast_text = ast_output.read_text("utf-8")
    assert "function" in ast_text
    assert ast_text.strip()
